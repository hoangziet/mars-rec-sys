...
==========================
RQ2 Stage A: tune watch_alpha for BERT4Rec-WL (weighted loss).

Runs BERT4Rec with watch_mode="loss" at different alpha values.
Logs results to MLflow experiment 'mars_watch_alpha_tuning'.

Usage:
    uv run python scripts/rq2_tune_alpha.py
    uv run python scripts/rq2_tune_alpha.py --alphas 0.25 0.5 1.0 --seeds 42 123
"""
...
DEFAULT_ALPHAS = [0.25, 0.5, 1.0, 2.0]
DEFAULT_SEEDS = [42]
BACKBONE = "bert4rec"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="RQ2: BERT4Rec-WL watch_alpha tuning.")
    parser.add_argument("--data-dir", default="data/processed")
    parser.add_argument("--output-dir", default="experiments")
    parser.add_argument("--alphas", nargs="+", type=float, default=DEFAULT_ALPHAS)
    parser.add_argument("--seeds", nargs="+", type=int, default=DEFAULT_SEEDS)
    parser.add_argument("--benchmark-id", default="rq2-watch-alpha")
...
def _run_single(args, alpha: float, seed: int) -> dict:
    ...
    model_kwargs["watch_mode"] = "loss"
    model_kwargs["watch_alpha"] = float(alpha)
    ...
    mlflow_cfg["tags"]["watch_alpha"] = str(alpha)
    mlflow_cfg["tags"]["watch_mode"] = "loss"
