...
for _, row in user_sequences.iterrows():
    ...
    uid = row["user_idx"]
    n   = len(seq)
    train_data.append({"user_idx": uid, "item_sequence": seq[:-2], "seq_len": n - 2})
    val_data.append(  {"user_idx": uid, "train_seq": seq[:-2], "target": seq[-2]})
    test_data.append( {"user_idx": uid, "train_seq": seq[:-1], "target": seq[-1]})
