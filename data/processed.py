...
# Force user_id and item_id to str so all merge/join keys are consistent
...
print(
    f"Implicit: {len(implicit):,} rows | {implicit['user_id'].nunique():,} users | {implicit['item_id'].nunique():,} items"
)
...
print(
    f"Before dedup: {len(implicit):,} rows -> After dedup: {len(implicit_dedup):,} rows"
)
...
user_sequences = user_sequences[user_sequences["seq_len"] >= MIN_SEQ_LEN].reset_index(
    drop=True
)
after = len(user_sequences)
print(f"Users còn lại: {after:,} / {before:,} ({after / before:.1%})")
print(
    f"Seq len — min: {user_sequences.seq_len.min()}, max: {user_sequences.seq_len.max()}, mean: {user_sequences.seq_len.mean():.1f}"
)
...
implicit_dedup["confidence"] = (
    implicit_dedup["watch_pct"].fillna(1.0).clip(0, 100) / 100.0
)
merged_count = implicit_dedup["watch_pct"].notna().sum()
print(
    f"Interactions có watch_pct: {merged_count:,} / {len(implicit_dedup):,} ({merged_count / len(implicit_dedup):.1%})"
)
print(
    "Confidence aggregation rule: explicit duplicates use mean watch_percentage; implicit duplicates keep earliest created_at row"
)
...
all_items = sorted(
    implicit_dedup[implicit_dedup["user_id"].isin(all_users)]["item_id"].unique()
)
...
for _, row in user_sequences.iterrows():
    ...
    val_data.append(
        {"user_idx": uid, "train_seq": train_history, "target": validation_target}
    )
