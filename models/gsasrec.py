...
gsasrec.py
==========
gSASRec — SASRec with confidence weighting from explicit feedback.

Architecture:
    - Same as SASRec (causal self-attention blocks)
    - Weighted cross-entropy loss using watch_percentage confidence
"""
...
def build_confidence_map(interactions_csv):
    df = pd.read_csv(interactions_csv)
    return {
        (int(r.user_idx), int(r.item_idx)): float(r.confidence)
        for r in df.itertuples(index=False)
    }


def weighted_ce_loss(logits, targets, confidence):
    per_sample = F.cross_entropy(logits, targets, reduction="none")
    return (per_sample * confidence).mean()
